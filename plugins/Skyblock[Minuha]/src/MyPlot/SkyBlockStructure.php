...
class SkyBlockStructure extends Populator{
	...
	public static function placeObject(ChunkManager $level, $chunk, $Xofchunk, $Zofchunk){
		...
		for($x = 4; $x < 11; $x++){
			for($z = 4; $z < 11; $z++){
				$level->setBlockIdAt($vec->x + $x, 68, $vec->z + $z, Block::NETHERRACK);
			}
		}
		for($x = 5; $x < 10; $x++){
			for($z = 5; $z < 10; $z++){
				$level->setBlockIdAt($vec->x + $x, 67, $vec->z + $z, Block::NETHERRACK);
		...
		for($x = 6; $x < 9; $x++){
			for($z = 6; $z < 9; $z++){
				...
				$level->setBlockIdAt($vec->x + $x, 66, $vec->z + $z, Block::NETHERRACK); // 66
			}
		}
		$level->setBlockIdAt($vec->x + 7, 64, $vec->z + 7, Block::BEDROCK); // 0
		$level->setBlockIdAt($vec->x + 7, 65, $vec->z + 7, Block::NETHERRACK); // 1
		$level->setBlockIdAt($vec->x + 7, 66, $vec->z + 7, Block::NETHERRACK); // 2
		$level->setBlockIdAt($vec->x + 7, 67, $vec->z + 7, Block::NETHERRACK); // 3
		$level->setBlockIdAt($vec->x + 9, 68, $vec->z + 8, Block::NETHERRACK);
		$level->setBlockIdAt($vec->x + 9, 68, $vec->z + 9, Block::NETHERRACK);
		$level->setBlockIdAt($vec->x + 9, 68, $vec->z + 7, Block::NETHERRACK);
		...
		$level->setBlockIdAt($vec->x + 7, 65, $vec->z + 8, Block::NETHERRACK); // 65
		$level->setBlockIdAt($vec->x + 8, 65, $vec->z + 7, Block::NETHERRACK);
		$level->setBlockIdAt($vec->x + 7, 65, $vec->z + 6, Block::NETHERRACK);
		$level->setBlockIdAt($vec->x + 6, 65, $vec->z + 7, Block::NETHERRACK);
		$level->setBlockIdAt($vec->x + 5, 66, $vec->z + 7, Block::NETHERRACK); // 66
		$level->setBlockIdAt($vec->x + 7, 66, $vec->z + 5, Block::NETHERRACK);
		$level->setBlockIdAt($vec->x + 9, 66, $vec->z + 7, Block::NETHERRACK);
		$level->setBlockIdAt($vec->x + 7, 66, $vec->z + 9, Block::NETHERRACK);
		$level->setBlockIdAt($vec->x + 4, 67, $vec->z + 7, Block::NETHERRACK); // 67
		$level->setBlockIdAt($vec->x + 7, 67, $vec->z + 4, Block::NETHERRACK);
		$level->setBlockIdAt($vec->x + 7, 67, $vec->z + 10, Block::NETHERRACK);
		$level->setBlockIdAt($vec->x + 10, 67, $vec->z + 7, Block::NETHERRACK);
	}
