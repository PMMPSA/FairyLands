...
class Bank extends PluginBase implements Listener{
	...
	public function onEnable(){
		@mkdir($this->getDataFolder());
                ...
                $this->reloadConfig();
		$this->getServer()->getLogger()->info("§8[§a Ngân Hàng §8] §eĐang tìm plugin EconomyAPI...");
		...
		if(!($this->money = $pm->getPlugin("PocketMoney")) && !($this->money = $pm->getPlugin("EconomyAPI")) && !($this->money = $pm->getPlugin("MassiveEconomy")) && !($this->money = $pm->getPlugin("Money"))){
			$this->getServer()->getLogger()->info("§8[§a Ngân Hàng §8] §bKhông tìm thấy plugin EconomyAPI.");
		}else{
			$this->getServer()->getLogger()->info("§8[§a Ngân Hàng §8] §bĐã tìm thấy plugin EconomyAPI!!!§a Sử dụng: " . $this->money->getName());
		}
		$this->touch = [];
	...
	public function onCommand(CommandSender $sender, Command $cmd, $label, array $sub){
		...
		if(!$sender instanceof Player){
			$r = $mm . ("§aDùng lệnh này tại in-game");
